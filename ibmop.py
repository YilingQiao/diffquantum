...
class QubitControl(object):
    ...
    def prefix_suffix(self, H0, Hs, M, initial_state, sample_time, len_t) :
        H = [H0]
        for i in range(len(Hs.keys())):
            ham = Hs[i]['H']
        ...
        for idx in range(M.shape[0]) :
            if abs(self.M_evals[idx]) > 1e-5 :
                ...
                for i in range(len(sample_time)-1, -1, -1) :
                    pre_s = len_t if i == len(sample_time)-1 else sample_time[i + 1]
                    s = sample_time[i]
                    psi = self.my_solver(H, psi, pre_s, s)
    ...
    def get_derivative_presuf(self, H0, Hs, s, sidx, len_t) :
        sgm = torch.nn.Sigmoid()
        for i in range(len(Hs.keys())):
            channels = Hs[i]['channels']
            for chan in channels:
                ...
                if self.basis == 'Legendre':
                    coeff_A = [self.vv[0,idx,j] * self.legendre_ps[j](2 * s / len_t - 1) \
                                    for j in range(self.n_basis)]
                    coeff_B = [self.vv[1,idx,j] * self.legendre_ps[j](2 * s / len_t - 1) \
                                    for j in range(self.n_basis)]
                elif self.basis == 'BSpline':
                    coeff_A = [self.vv[0,idx,j] * self.func_bsplines[j](s / len_t) \
                                    for j in range(self.n_basis)]
                    coeff_B = [self.vv[1,idx,j] * self.func_bsplines[j](s / len_t) \
                                    for j in range(self.n_basis)]
    ...
    def compute_energy_grad_MC(self, H0, Hs, M, initial_state, len_t=None):
        ...
        if self.is_sample_uniform:
            sample_time = [
            (ss + 1.0) * len_t / (num_sample + 1) for ss in range(num_sample)]
        elif self.is_sample_discrete == False:
            sample_time = np.random.uniform(0, len_t, size=num_sample)
        else: 
            sample_time = 1 + np.random.randint(0, len_t-1, size=num_sample)

        sample_time.sort()
        self.sample_time = sample_time
        if not self.sampling_measure :
            self.prefix_suffix(H0, Hs, M, initial_state, sample_time, len_t)
        ...
        for sidx, s in enumerate(sample_time):
            if self.vv.grad != None :
                self.vv.grad.zero_()
            if self.sampling_measure :
                grad += self.get_integrand(H0, Hs, M, initial_state, s, len_t)
            else :
                grad += self.get_derivative_presuf(H0, Hs, s, sidx, len_t)

        
        return torch.from_numpy(len_t * grad / num_sample)
    ...
    def compute_energy(self, H0, Hs, M, initial_state, len_t=None):
        if len_t is None:
            len_t = self.duration
        ...
        psi_T = self.my_solver(H, initial_state, 0, len_t)
    ...
    def train_fidelity(self, vv0, H0, Hs, initial_states, target_states, ts):
        """Train the sepctral coefficients to minimize energy minimization.
        Args:
            M: A Hermitian matrix.
            initial_state: initial_state.
        ...
        for epoch in range(1, self.n_epoch + 1):
            ...
            for i in idxs:
                ...
                len_t = ts[i]
                M = I - psi1 * psi1.dag() 
                loss_fidelity = self.compute_energy(H0, Hs, M, psi0, len_t)
                grad_vv += self.compute_energy_grad_MC(H0, Hs, M, psi0, len_t)
    ...
    def demo_Heis3(self):
        self.logger.write_text("demo_Heis3 ========")
        n_qubit = 3
        self.n_qubit = n_qubit

        Heis3 = self.multi_kron(*[self.O for j in range(n_qubit)])
        for i in range(2):
            Heis3 += self.multi_kron(*[self.I if j not in [i, i + 1] else self.X for j in range(n_qubit)])
            Heis3 += self.multi_kron(*[self.I if j not in [i, i + 1] else self.Y for j in range(n_qubit)])
            Heis3 += self.multi_kron(*[self.I if j not in [i, i + 1] else self.Z for j in range(n_qubit)])

        ts = np.linspace(0, 1, 5) * np.pi * 20
        ts = ts[1:]
        Us = [scipy.linalg.expm(-1.j * t * Heis3) for t in ts]
        self.duration = ts[-1]
        ...
        psi0 = np.kron(np.kron(e, e), g)
        initial_states = [psi0 for t in ts]
        target_states = [np.matmul(Us[i], psi0) for i in range(len(ts))]
        ...
        self.train_fidelity(vv0, H0, Hs, initial_states, target_states, ts)
...
if __name__ == '__main__':
    np.random.seed(0)
    model = QubitControl(
        basis='Legendre', n_basis=1, dt=0.22222222222, 
        duration=1200, n_epoch=2000, lr = 3e-2, num_sample=10, per_step=200, solver=0, sampling_measure=False)
